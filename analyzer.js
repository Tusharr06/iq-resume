...
class ResumeAnalyzer {
  ...
  setupEventListeners() {
    // Method selection
    this.uploadMethodBtn.addEventListener("click", () =>
      this.switchMethod("upload")
    );
    this.textMethodBtn.addEventListener("click", () =>
    ...
    // Preview toggle
    this.togglePreview.addEventListener("click", () =>
      this.togglePreviewContent()
    );
  ...
  async processFile(file) {
    ...
    try {
      ...
      if (error.message.includes("fetch")) {
        alert(
          "Could not connect to the analysis server. Please make sure the Python API server is running on localhost:5000"
        );
      } else {
  ...
  downloadResults() {
    ...
    const categoryText =
      categoryElement.textContent || categoryElement.innerText;

    const results = {
      timestamp: new Date().toISOString(),
      predictedCategory: categoryText.split("Confidence:")[0].trim(),
      confidence: categoryText.includes("Confidence:")
        ? categoryText.split("Confidence:")[1].trim()
        : "N/A",
      extractedText:
        this.currentText.substring(0, 1000) +
        (this.currentText.length > 1000 ? "..." : ""),
